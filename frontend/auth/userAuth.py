...
class UserRegistrationForm(FlaskForm):
    ...
    team = SelectFiled('Team', choices=const.TEAM_CHOICES)
...
def user_register():
    ...
    if reg_form.validate_on_submit():
        ...
        if _is_reg:
            ...
            flash('User: ' + reg_form.email.data + ' already exists for Shift Plan App!! Please log-in', category='info')
            return redirect(url_for('auth.user_login'))
        else:
    ...
    if login_form.validate_on_submit():
        ...
        if user_login_data:
            ...
            if user_login_data['verify_username'] and user_login_data['verify_pass']:
                log.debug('session cookie: ' + session)
